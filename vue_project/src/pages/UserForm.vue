...
const submitForm = async () => {
    ...
    try {
        ...
        if (props.task === 'add') {
            ...
            await services.users.create(access, { ...form.value })
            alert(`User created successfully!`)

        } else {
            await services.users.update(access, form.value.user_id, { ...form.value })
            alert(`User altered successfully!`)
        }

        const response = await services.users.list(access)
